...
cp "$HOSTFILE" "$OUT/hosts_router_3.txt"
{
  ...
  echo "mpi_started_at=$(date -Is)"
  echo "seq_reused=true"
  echo "root=$ROOT"
  echo "binary=$BIN"
...
echo "MPI_ONLY_SMOKE $(date -Is)" >> "$OUT/progress.log"
...
echo "mpi_finished_at=$(date -Is)" >> "$OUT/meta.txt"
find "$OUT" -maxdepth 1 -name '*.json' | wc -l > "$OUT/json_count.txt"
echo "MPI_ONLY_DONE $(date -Is)" >> "$OUT/progress.log"
